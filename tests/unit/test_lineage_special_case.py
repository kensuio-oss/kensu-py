...
from kensu.client import ApiClient, DataSource, DataSourcePK, SchemaPK, Schema, FieldDef, ProcessLineagePK, ProcessRef, \
    ProcessLineage, SchemaRef, SchemaLineageDependencyDef
...
class TestLineageSpecialCase(unittest.TestCase):
    ...
    def test_one(self):
        ...
        self.report_lineage(schema1, ds2_app1_schema, {'col1': ['col1']})
    ...
    def report_lineage(self, from_schema, to_schema, column_data_deps):
        dataflow = [
        ...
        return ProcessLineage(name='Lineage from {} to {}'.format(from_schema.name, to_schema.name, ),
                              operation_logic='APPEND',
                              pk=ProcessLineagePK(process_ref=ProcessRef(by_guid=self.kensu_tracker.process.to_guid()),
