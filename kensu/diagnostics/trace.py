# https://github.com/python/cpython/blob/3.9/Lib/trace.py
...
#
# Modified: 202108 by Stanislas Rusinsky
#
...
class CoverageResults:
    ...
    def write_results(self, show_missing=True, summary=False, coverdir=None):
        ...
        if self.calledfuncs:
            ...
            print("STAAAAAN %s", self.calledfuncs)
            sys.exit(-1)
            for filename, modulename, funcname, count in sorted(calls):
                print(("filename: %s, modulename: %s, funcname: %s, count: %s"
                       % (filename, modulename, funcname, count)))
...
class Trace:
    ...
    def globaltrace_countfuncs(self, frame, why, arg):
        ...
        if why == 'call':
            this_func = self.file_module_function_of(frame)
            if this_func in self._calledfuncs:
                if isinstance(self._calledfuncs[this_func], int):
